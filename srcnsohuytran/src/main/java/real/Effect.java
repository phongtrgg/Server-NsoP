...
public class Effect {
    ...
    public static boolean isPermanentEffect(final @NotNull Effect effect) {
        ...
        return type == 0 || type == 18 || type == 25 || type == 26  || type == 27 || type == 28 || type == 20 || type == 21  || type == 22 || type == 23 || type == 24;
    }
    ...
    public static Effect fromJSONObject(final  @NotNull JSONObject jsonObject) {
        ...
        if (effTemplate.id == 36 || effTemplate.id == 37 || effTemplate.id == 38 || effTemplate.id == 39 || effTemplate.id == 42) {
            object.timeRemove = Long.parseLong(jsonObject.get(TIME_REMOVE).toString());
        } else {
            object.timeRemove = Long.parseLong(jsonObject.get(TIME_REMOVE).toString()) - Long.parseLong(jsonObject.get(TIME_SAVE).toString()) + System.currentTimeMillis();
        }
        return object;
    }
