...
//                        wait(TIME_TO_DISCONNECT);
    ...
    private void flushClanData() throws IOException {
        ...
        for (ClanManager entry : ClanManager.entrys) {
            if (entry != null) {
                for (int i = 0; i < entry.clanThanThus.size(); i++) {
                    ClanThanThu thanThu = entry.clanThanThus.get(i);
                    if (thanThu != null) {
                        val item = thanThu.getPetItem();
                        if (item.isExpires && item.expireTrung() && (item.id == TRUNG_DI_LONG_ID || item.id == TRUNG_HAI_MA_ID)) {
                            entry.clanThanThus.remove(i);
                            if (item.id == TRUNG_DI_LONG_ID) {
                                if (entry.containThanThu(0)) {
                                    ...
                                    entry.sendMessage(entry.createMessage("Gia tộc bạn nhận được dị long cấp 1"));
