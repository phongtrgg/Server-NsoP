    ...
    public @NotNull short @NotNull [] getRewards(@NotNull IGlobalBattler battle) {
        try {
            if (this.state == START_STATE) {
                battle.notifyMessage("Chiến trường đang diễn ra. Vui lòng đợi kết thúc để nhận thưởng.");
                return new short[0];
            } else if ((battle.getPhe() != Constants.PK_TRANG && battle.getPhe() != Constants.PK_DEN) || battle.getPoint() < 1000) {
                return new short[0];
            } else {
