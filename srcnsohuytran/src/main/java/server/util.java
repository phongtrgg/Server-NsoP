...
public class util {
    ...
    public static synchronized boolean compare_Sec(final Date now, final Date when) {
        try {
            final Date date1 = util.dateFormat.parse(util.dateFormat.format(now));
            final Date date2 = util.dateFormat.parse(util.dateFormat.format(when));
            return !date1.equals(date2) && !date1.before(date2);
        }
        catch (ParseException p) {
            p.printStackTrace();
